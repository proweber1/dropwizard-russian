...
`#sayHello(Optional<String>)` является самым интересным методом этого класса и является достаточно простым.
`@QueryParam("name")` аннотация сообщает Jersey, чтобы нужно отобразить query параметр
`name` на `name` параметр метода. Если клиент шлет запрос `/hello-world?name=Dougie` то
метод `sayHello` будет вызван с `Optional.of("Dougie)`; а если параметр
`name` не передан в Query строке, то метод `sayHello` будет вызван с
`Optional.absent()`.

> **От переводчика**:
> В данном контексте класс Optional это не класс java.util.Optional это класс из
> библиотеки Guava

> **Заметка**:
> Если клиент пошлет следующий запрос `/hello-world?name=`, метод `sayHello`
> будет вызван с `Optional.of("")`. Это может показаться странным по началу, но
> это стандартное поведение. Чтобы такого не допустить вам нужно изменить тип параметра в методе
> с `Optional<String>` на `NonEmptyStringParam`, и тогда при выполнении запроса
> `/hello-world?name=` будет возвращена строка `Hello, Stranger!`.

Внутри метода `sayHello` мы инкрементируем счетчик, форматируем шаблон ответа с помощью
`String.format(String, Object...)`, и возвращаем новый объект `Saying`.

Так как метод `sayHello` аннотирован `@Timed`, DropWizard автоматически записывает
продолжительность и скорость исполнения этого метода с помощью `Metrics Timer`.

После того как наш метод вернул экземпляр объекта `Saying`, Jersey получает его и ищет объект который может
его сериализовать, который отвечает за `application/json` сериализацию.

Дальше этот провайдер сериализует наш POJO в JSON и отдает его клиенту со статус кодом
`200` и `content-type: application/json; charset=utf-8`.

### Регистрация ресурса в приложении

Перед тем как это действительно заработает, нам нужно вернуться к классу `HelloWorldApplication` и зарегистрировать
там наш ресурс. Для этого в методе `HelloWorldApplication#run` мы должны прочитать `defaultName` и `template` из файла
конфигурации, передать их в класс ресурса через конструктор и зарегистрировать ресурс в приложении.
...
когда наше приложение запускается, мы создаем новый экземпляр нашего ресурса с параметрами
из конфигурационного файла и передаем его в окружающую среду приложения которая действует как
реестр всех вещей которое ваше приложение может делать.

> **Заметка**:
> Dropwizard приложение может содержать много классов ресурсов, каждый из которых будет
> иметь соответствующий URI шаблон. Чтобы добавить другие ресурсы просто добавьте аннотацию
> `@Path` к классу ресурса и зарегистрируйте ресурс в окружении приложения как это было сделано
> выше.

Перед тем как запустить приложение, мы должны добавить health check для нашего приложения.
